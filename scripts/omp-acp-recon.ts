...
const program = Effect.gen(function* () {
  ...
  const child = yield* spawner.spawn(
    ...
    Effect.tapError((err) => Effect.sync(() => log("SPAWN_ERROR", err))),
  );

  log("RECON", "Process spawned, building ACP client layer…");
  ...
  const result = yield* Effect.gen(function* () {
    ...
    yield* acp.handleReadTextFile((req: AcpSchema.ReadTextFileRequest) =>
      Effect.gen(function* () {
        log("HANDLER:fs/read_text_file", { path: req.path });
        ...
        log("HANDLER:fs/read_text_file:RESULT", {
          path: req.path,
    ...
    yield* acp.handleWriteTextFile((req: AcpSchema.WriteTextFileRequest) =>
      Effect.sync(() => {
        ...
        // Return void; the type allows void per the client.ts signature
      }),
    );
    ...
    yield* acp.handleRequestPermission((req: AcpSchema.RequestPermissionRequest) =>
      Effect.sync(() => {
        log("HANDLER:session/request_permission:AUTO_APPROVE", req);
        return {
          outcome: {
    ...
    yield* acp.handleCreateTerminal((req: AcpSchema.CreateTerminalRequest) =>
      Effect.sync(() => {
        log("HANDLER:terminal/create:STUB", req);
        return { terminalId: `stub-terminal-${Date.now()}` };
      }),
    );
    ...
    yield* acp.handleTerminalOutput((req: AcpSchema.TerminalOutputRequest) =>
      Effect.sync(() => {
        log("HANDLER:terminal/output:STUB", { terminalId: req.terminalId });
    ...
    log("RESULT:initialize", initResult);
    ...
    log("INFO:authMethods", authMethods ?? "(none advertised)");
