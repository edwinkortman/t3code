...
/**
 * OMP speaks standard Zed ACP — no xAI-specific _meta stop-reason injection.
 * The response's stopReason is always the definitive value.
 */
...
export function makeOmpAdapter(ompSettings: OmpSettings, options?: OmpAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const startSession: OmpAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // OMP speaks standard Zed ACP — no x.ai/ask_user_question extension
          // handlers are registered here. If omp ever adds a vendor extension
          // request, add a handleExtRequest call before acp.start() following
          // the GrokAdapter pattern (GrokAdapter.ts:611–664).
    ...
    /**
     * OMP speaks standard Zed ACP and does not emit vendor user-input extension
     * requests (recon confirmed: no x.ai/ask_user_question equivalent observed).
     * If omp adds a user-question extension in a future version, re-implement
     * this following the GrokAdapter pattern (GrokAdapter.ts:1375–1391) and
     * add the corresponding handleExtRequest call in startSession.
     *
     * TODO(omp-recon): re-verify once a model is configured and a full turn
     * completes — it is possible a user-confirmation callback surfaces only
     * during destructive operations (see omp-adapter-spec.md §3.5).
     */
    const respondToUserInput: OmpAdapterShape["respondToUserInput"] = (
      ...
      Effect.fail(
        new ProviderAdapterRequestError({
          ...
          detail:
            "OMP ACP does not support user-input requests (no ask_user_question extension observed).",
        }),
      );
