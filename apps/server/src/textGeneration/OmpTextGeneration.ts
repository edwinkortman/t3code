...
} from "../provider/acp/OmpAcpSupport.ts"; // TODO(omp-wave1): OmpAcpSupport already exists — no gap here

// TODO(omp-recon): confirm whether omp exposes a non-ACP text-generation path;
// if not, route through the ACP prompt runtime (current approach).
// omp text generation rides the ACP session runtime exclusively — there is no
// separate HTTP/completion API as far as is currently known. The XAi
// PromptCompletionRuntime wrapper used by Grok is intentionally omitted here
// because that is xAI-specific (see OmpAcpSupport.ts line 80).
...
export const makeOmpTextGeneration = Effect.fn("makeOmpTextGeneration")(function* (
  ...
  const runOmpJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      // TODO(omp-wave1): Layers/OmpAdapter.ts not yet created — makeOmpAcpRuntime
      // is available from OmpAcpSupport.ts; adapter layer wiring comes in Wave 1.
